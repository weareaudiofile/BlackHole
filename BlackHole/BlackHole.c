...
//    UInt32 p = inAddress->mSelector;
//    DebugMsg("zzz bh has property %c%c%c%c?", p>>24, p>>16, p>>8, p&0xff);
//
...
//    DebugMsg("zzz answer: %i for %c%c%c%c?", theAnswer, p>>24, p>>16, p>>8, p&0xFF);
...
//            DebugMsg("format: %i", x[i]);
    ...
    } else if(kAudioDevicePropertyZeroTimeStampPeriod == inAddress->mSelector || kAudioDevicePropertyIsHidden == kAudioDevicePropertyZeroTimeStampPeriod == inAddress->mSelector || kAudioStreamPropertyStartingChannel == inAddress->mSelector || kAudioDevicePropertyIsHidden == inAddress->mSelector || kAudioDevicePropertyDeviceCanBeDefaultDevice == inAddress->mSelector || kAudioDevicePropertyDeviceCanBeDefaultSystemDevice == inAddress->mSelector || kAudioBooleanControlPropertyValue == inAddress->mSelector || kAudioDevicePropertySafetyOffset == inAddress->mSelector || kAudioDevicePropertySafetyOffset == inAddress->mSelector) {
        UInt32 x = *(UInt32*)(outData);
        DebugMsg(" --> %i", x);
...
Done:
//    DebugMsg("zzz zero hanswer: 0x%x", theAnswer);
	return theAnswer;
}
...
static OSStatus	BlackHole_WillDoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, Boolean* outWillDo, Boolean* outWillDoInPlace)
{
	...
	//	declare the local variables
	OSStatus theAnswer = 0;
//    DebugMsg("will do io %i", inOperationID);
...
//    DebugMsg("will begin io %i", inOperationID);
...
static OSStatus	BlackHole_EndIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo)
{
	...
	#pragma unused(inClientID, inOperationID, inIOBufferFrameSize, inIOCycleInfo)
//    DebugMsg("will END IO %i", inOperationID);
