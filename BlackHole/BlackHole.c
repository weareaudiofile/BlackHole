...
static Boolean    BlackHole_HasProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress)
{
    ...
    char st[5] = {0};
    st[3] = inAddress->mSelector & 0xff;
    st[2] = (inAddress->mSelector >> 8) & 0xff;
    st[1] = (inAddress->mSelector >> 16) & 0xff;
    st[0] = (inAddress->mSelector >> 24) & 0xff;
    DebugMsg("BlackHole_HasProperty: AudioObjectID: %i AudioObjectPropertyAddress: %s", inObjectID, st);
...
static OSStatus    BlackHole_IsPropertySettable(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, Boolean* outIsSettable)
{
    ...
    st[1] = (inAddress->mSelector >> 16) & 0xff;
    st[0] = (inAddress->mSelector >> 24) & 0xff;
    DebugMsg("BlackHole_IsPropertySettable: AudioObjectID: %i AudioObjectPropertyAddress: %s", inObjectID, st);
...
static OSStatus    BlackHole_GetPropertyDataSize(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32* outDataSize)
{
    ...
    st[1] = (inAddress->mSelector >> 16) & 0xff;
    st[0] = (inAddress->mSelector >> 24) & 0xff;
    DebugMsg("BlackHole_GetPropertyDataSize: AudioObjectID: %i AudioObjectPropertyAddress: %s", inObjectID, st);
...
static OSStatus    BlackHole_GetPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
    ...
    st[1] = (inAddress->mSelector >> 16) & 0xff;
    st[0] = (inAddress->mSelector >> 24) & 0xff;
    DebugMsg("BlackHole_GetPropertyData: AudioObjectID: %i AudioObjectPropertyAddress: %s", inObjectID, st);
    //    declare the local variables
    OSStatus theAnswer = 0;
...
static OSStatus    BlackHole_SetPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, const void* inData)
{
    ...
    st[1] = (inAddress->mSelector >> 16) & 0xff;
    st[0] = (inAddress->mSelector >> 24) & 0xff;
    DebugMsg("BlackHole_SetPropertyData: AudioObjectID: %i AudioObjectPropertyAddress: %s", inObjectID, st);
    ...
    switch(inAddress->mSelector)
    {
        ...
        case kAudioObjectPropertyOwnedObjects:
            ...
            if(theNumberItemsToFetch > 1)
            {
                ...
                ((AudioObjectID*)outData)[1] = kObjectID_Device; // BUG? originally listed as [0]
            }
...
//            for(theItemIndex = 0; theItemIndex < theNumberItemsToFetch; ++theItemIndex)
//            {
//                // kind of a dumb way to do this. Just not very readable.
//                if(theItemIndex < 3)
//                {
        ...
        case kObjectID_DataSource_Output_Master:
            ...
            case kObjectID_ClockSource:
                switch(inAddress->mSelector)
                {
                    ...
                    case kAudioSelectorControlPropertyAvailableItems:
                    {
                        DebugMsg("BlackHole kAudioSelectorControlPropertyAvailableItems");
...
//
//                        kClockSource_NumberItems = dataSize/sizeof(UInt32);
//
//
...
//                    AudioObjectGetPropertyData(kAudioObjectSystemObject, &addr, 0, NULL, &dataSize, devices);
                    ...
                    {
                        ((UInt32*)outData)[theItemIndex] = theItemIndex;
//                        ((UInt32*)outData)[theItemIndex] = devices[theItemIndex];
                    }
                ...
                case kAudioSelectorControlPropertyItemName:
                    ...
                    // get BlackHole Audio ID
                    // if we try to access HAL using BlackHole Audio ID it will hang.
                    // what I should do is lock this down so we are using Aggregate Devices.
//                    AudioObjectPropertyAddress inAddr;
//                    inAddr.mSelector = kAudioHardwarePropertyTranslateUIDToDevice;
//                    UInt32 inQualifierDataSize1 = sizeof(CFStringRef);
//                    CFStringRef inQualifierData1 = CFSTR(kDevice_UID);
//                    UInt32 ioDataSize = sizeof(AudioObjectID);
//                    AudioObjectID blackHoleID;
                    
//                    AudioObjectGetPropertyData(kAudioObjectSystemObject, &inAddr, inQualifierDataSize1, &inQualifierData1, &ioDataSize, &blackHoleID);
                    
//                    if (*(UInt32*)inQualifierData == blackHoleID)
//                    {
//                        // return the name for BlackHole
//                        *(CFStringRef*)outData = CFStringCreateWithFormat(NULL, NULL, CFSTR(kDataSource_ItemNamePattern), NUMBER_OF_CHANNELS);
//                    }
//                    else
//                    {
//
//                        // return all the other audio device names
//                         AudioObjectPropertyAddress address;
//                         address.mSelector = kAudioDevicePropertyDeviceNameCFString;
//                        UInt32 size = sizeof(CFStringRef);
//                        AudioDeviceID audioID = *(UInt32*)inQualifierData;
//
//                         OSStatus error = AudioObjectGetPropertyData(audioID, &address, 0, NULL, &size, outData);
//                        if (error)
//                        {
//                            DebugMsg("BlackHole kAudioDevicePropertyDeviceNameCFString error: %i", error);
//                        }
//
//                    }
